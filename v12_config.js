...
const V12_CONFIG = {
  ...
  SETTINGS: {
    ...
    // Имя свойства ПОЛЬЗОВАТЕЛЯ (PropertiesService.getUserProperties), в котором
    // хранится e-mail, указанный пользователем вручную.
    //
    // Зачем: права доступа (RBAC) определяются по e-mail, а
    // Session.getActiveUser().getEmail() не отдаёт адрес чужого (не владельца)
    // аккаунта в исполнении по триггеру — для снабженца это выглядело как
    // «нет доступа на правку» в «Сводке дефицитов». Мастер делает то же, что
    // сателлит отборщика: при недоступном системном e-mail актор указывается
    // явно и сверяется с V12_ROLE_MAP.
    USER_EMAIL_PROPERTY: "V12_USER_EMAIL",
    // Префикс ключа ОБЩЕГО реестра «анонимный ключ пользователя → e-mail».
    //
    // Зачем реестр, если уже есть USER_EMAIL_PROPERTY. Свойства пользователя
    // доступны «текущему ИЛИ эффективному пользователю»: в исполнении по
    // установленному onEdit-триггеру эффективный пользователь — владелец
    // таблицы, поэтому e-mail, который ЭКОНОМИСТ указал в своих настройках, в
    // триггере не виден, и права снова не определялись (галочка «Реальная
    // поставка» откатывалась). В триггере платформа отдаёт только АНОНИМНЫЙ
    // ключ пользователя (Session.getTemporaryActiveUserKey) — по нему и
    // раскладывается реестр: он общий для скрипта, но не раскрывает личность.
    USER_EMAIL_REGISTRY_PREFIX: "V12_USER_EMAIL_BY_KEY_",
    // Срок жизни записи реестра, дней: анонимный ключ платформа меняет раз в
    // 30 дней, поэтому хранить запись дольше бессмысленно (протухшая запись
    // просто не найдётся по новому ключу).
    USER_EMAIL_REGISTRY_TTL_DAYS: 30
  },

 ...
 * «BOM (кол-во)», «Всего непокрыто» и «Обновлено».
 */
const V12_SUPPLY_LEGACY_HEADERS = ["Всего требуется", "Всего зарезервировано", "BOM (кол-во)", "Всего непокрыто", "Обновлено"];

/**
 * Миграция листа СНАБЖЕНИЕ под новую схему.
 *
 * В прежних схемах СНАБЖЕНИЕ содержало колонки «Всего требуется», «Всего
 * зарезервировано», «BOM (кол-во)», «Всего непокрыто» и «Обновлено». Из канона
 * (9 колонок) они убраны. Миграция удаляет любые устаревшие колонки из заголовка
 * и приводит строку заголовков к канону. Тело листа не трогаем — оно
 * пересобирается v12RefreshSupply.
 */

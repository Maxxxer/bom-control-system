 ...
 *   F7 — при занятом локе (идёт Apply) намерение фиксируется АТОМАРНОЙ
 *        дозаписью (appendRow): отметка отборщика НЕ теряется, очередь не
 *        портится дублями. Прежний «безопасный отказ» означал молчаливую
 *        потерю отметки — для отборщика это худший дефект;
...
// Слив применяет намерение «от имени» записанного в очереди пользователя
// (v12WithActor), поэтому плейсхолдер-пользователи теста тоже ADMIN.
V12_ROLE_MAP[globalThis.__testUser || "test@example.com"] = C.ROLES.ADMIN;
["u", "u1", "u2", "u3"].forEach(function (u) { V12_ROLE_MAP[u] = C.ROLES.ADMIN; });
...
globalThis.__lockBusy = false;
check("F7: отметка C2 не потеряна", (function () {
  return pendingRows().some(function (r) { return r[Q.POSITION_ID - 1] === "BOM1:C2"; });
})(), true);
check("F7: ранее записанная отметка C1 сохранена", (function () {
  return pendingRows().some(function (r) { return r[Q.POSITION_ID - 1] === "BOM1:C1"; });
})(), true);
check("F7: добавлена ровно одна строка", bodyRows(), before + 1);
check("F7: живых намерений ровно два", pendingRows().length, 2);
